...
These two classes are the building blocks for
the BST, AVLTree, and Heap data structures.
It is crucial to get these implemented correctly
in order to be able to implement the other data
structures.
'''
...
class Node():
    ...
    Given a node t, you can visualize the node by
    running str(t) in the python interpreter.
...
class BinaryTree():
    ...
    def __init__(self, root=None):
        ...
        Note that for an ordinary BinaryTree, we
        cannot insert more than one element in
        the constructor,
    ...
    def print_tree(self, traversal_type):
        '''
        There are three primary types of tree
        traversals: preorder, inorder, and postorder.
        All three of these traversals are implemented
        for you as a reference on how to write
        recursive functions on recursive data
        structures.
        '''
        if traversal_type == 'preorder':
            ...
            raise ValueError('Type ' + str(traversal_type) + 'not supported.')
    ...
    def to_list(self, traversal_type):
        ...
        A general programming principle is that a
        function should return its results
    ...
    def _height(node):
        ...
        return 1 (for the current node) plus the max of
        the left and right _heights calculated above
        '''
        if node is None:
            return -1
        a = BinaryTree._height(node.left)
        b = BinaryTree._height(node.right)
        return 1 + max(a, b)
